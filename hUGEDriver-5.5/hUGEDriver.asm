;;; My Frankestein hUGEDriver mod... Kinda hUGEDriver 5.5...?
;;; Includes:
;;;     - Software volume shifting
;;;     - Volume Slide replaced with inverted arpeggio
;;;     - Replaced note delay with rudementary 4 step groove system
;;;     - Speed 0 is the same as a halt command in FT (Cxx)
;;;     - run routine effect commented out
;;;     - 8xx changed so it rets out if any channels are muted
...
ticks_per_row: dl ; 4 bytes for a 4 step groove
groove_pointer: db ; which groove were in (0-3)
...
;;; Updates the software volume byte
;;; Param: C = Value loaded onto the software volume controller
;;; 0   = 100%
;;; 1   = 50%
;;; 2   = 25%
;;; 3   = 12.5%
;;; >=4 = 0%
;;; Destroy: AF C
hUGE_set_software_volume::
    ld a, c
    .skip:
    and $07 ; To avoid any instance of say, vol being 255 and slowing down the driver
    ld [software_volume], a
    ret
...
get_note_period:
    ;; Get sgb1 and add
    ;; ...Apparently DE is always modified instantly after this function is called, so I'm gonna byte the bullet and not preserve it.
    ...
    ld e, 144 ; Number of bytes to skip
    .nosgb:
    ENDC
    pop af
...
.jump:
    ...
    dw fx_set_groove_slot              ;7xy
    dw fx_set_pan                      ;8xy ; global
    dw fx_set_duty                     ;9xy
    dw fx_arpeggio_down                ;Axy
...
fx_set_pan:
    ...
    ;; To avoid any pan writes if sfx are due
    ld a, [mute_channels]
    or a
    ret nz
...
;;; Processes effect A, "arpeggio down".
;;; Param: B = Current channel ID (0 = CH1, 1 = CH2, etc.)
...
;;; Processes effect 7, "set groove slot".
;;; Param: B = Current channel ID (0 = CH1, 1 = CH2, etc.)
;;; Param: C = XX YYYYYY
;;;            ||-++++++- Speed to store (0-63)
;;;            ++-------- Slot to write to (0-3)
;;; Param: ZF = Set if and only if on tick 0
;;; Destroy: AF B C
fx_set_groove_slot:
    ret nz

    ; Get the two slot bits in the lower bits of A.
    ld a, c
    rlca
    rlca
    ; Index into the `ticks_per_row` table.
    and 3
    add a, LOW(ticks_per_row)
    ld e, a
    adc a, HIGH(ticks_per_row)
    sub e
    ld d, a
...
;;; Param: A = NRX2 Volume write
;;; Return: A = Modified NRX2 Write
;;; Destroy: AF
shift_volume:
    ; Store envelope nibble
    push bc
    push af
    
    ; Shift by software volume
    ld c, a
    ld a, [software_volume]
    or a
    jr nz, .zerocheckdone
    pop af
    pop bc
...
;;; Param: A = NR32 format 
;;; Return: A = NR32 formatted shift volume
get_shift_ch3:
    push bc
...
_hUGE_set_software_volume_banked::
IF DEF(BANKED_FUNCS)
    ld hl, sp+2+4
    jr continue_set_software_volume
ENDC
_hUGE_set_software_volume::
    ld hl, sp+2
continue_set_software_volume:
    push bc
    ld a, [hl]
    call hUGE_set_software_volume.skip
    pop  bc
    ret
