...
enum hUGE_soft_vol_t    {HT_SV_FULL = 0, HT_SV_HALF, HT_SV_QUAR, HT_SV_EIGH, HT_SV_SIXT, HT_SV_MUTE};
...
void hUGE_set_software_volume(enum hUGE_soft_vol_t softvol);
void hUGE_set_software_volume_banked(enum hUGE_soft_vol_t softvol) __banked;

unsigned char hUGE_stopped(); // This actually returns the current song speed, for which 0 would mean the song stopped and hUGE_dosound is in a return loop.

extern volatile unsigned char hUGE_current_wave;
extern volatile unsigned char hUGE_SGB;
